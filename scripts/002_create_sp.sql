...
@END_DATE DATETIME2,
@TOTAL_INCOME (DECIMAL(10,2)) OUTPUT,
@TOTAL_EXPENSE (DECIMAL(10,2)) OUTPUT,
@ERROR_ID INT OUTPUT,
@ERROR_DESCRIPTION (VARCHAR(MAX)) OUTPUT
AS BEGIN
	BEGIN TRY
		...
		SELECT 
			@TOTAL_INCOME = SUM(CASE WHEN TYPE = 'In' THEN AMOUNT ELSE 0 END),
			@TOTAL_EXPENSE = SUM(CASE WHEN TYPE = 'Ex' THEN AMOUNT ELSE 0 END)
		FROM TRANSACTION
...
-- SPXXX : SP_GET_TRANSACTIONS_BY_DATE

CREATE OR ALTER PROCEDURE SP_GET_TRANSACTIONS_BY_DATE
	@ID_USER INT,
	@START_DATE DATETIME2,
	@END_DATE DATETIME2,
	@PAGE_NUMBER INT,
...
BEGIN
	BEGIN TRY
        ...
        BEGIN
            ...
            RETURN;
        END
		IF(@END_DATE < @START_DATE OR @PAGE_NUMBER < 0)
		BEGIN
			SET @ERROR_ID = 50001;
			SET @ERROR_DESCRIPTION = 'WRONG_DATA'
			RETURN;
	    ...
	    DECLARE @OFFSET INT = (@PAGE_NUMBER - 1) * 50;

        -- GET RESULTS FROM PAGE
        SELECT ID_TRANSACTION, [TYPE], [DATE], AMOUNT, [DESCRIPTION]
        FROM [TRANSACTION]
        WHERE ID_USER = @ID_USER
        ...
        SET @HAS_NEXT_PAGE = CASE 
            WHEN EXISTS (
                ...
                WHERE ID_USER = @ID_USER
                    AND DATE BETWEEN @START_DATE AND @END_DATE
                ORDER BY DATE DESC
                OFFSET (@OFFSET + 50) ROWS FETCH NEXT 1 ROW ONLY
            ) THEN 1 ELSE 0 END;
