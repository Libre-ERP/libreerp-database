...
CREATE PROCEDURE SP_GET_TRANSACTIONS_BY_DATE
@ID_USER INT,
...
BEGIN
	BEGIN TRY
        ...
        BEGIN
            SET @ERROR_ID = 1;
            SET @ERROR_DESCRIPTION = 'PAGE_SMALLER_THAN_1';
            RETURN;
        END
        ...
        BEGIN
            SET @ERROR_ID = 2;
            SET @ERROR_DESCRIPTION = 'START_DATE_MUST_BE_LESS_THAN_OR_EQUAL_TO_END_DATE';
            RETURN;
        END

		IF NOT EXISTS (SELECT 1 FROM [TRANSACTION] WHERE ID_USER = @ID_USER)
		BEGIN 
			SET @ERROR_ID = 3;
            SET @ERROR_DESCRIPTION = 'USER_NOT_FOUND';
            RETURN;
		...
		SELECT TOP 50 ID_TRANSACTION, TYPE, DATE, AMOUNT, DESCRIPTION
		FROM TRANSACTION
		...
		SELECT @HAS_NEXT_PAGE = CASE 
			WHEN EXISTS (
				SELECT 1 
				FROM TRANSACTION 
				WHERE ID_USER = @ID_USER AND DATE BETWEEN @START_DATE AND @END_DATE
				ORDER BY DATE DESC
				OFFSET (@OFFSET + 50) ROWS FETCH NEXT 1 ROWS ONLY
			) THEN 1 ELSE 0 END;
	END TRY
	BEGIN CATCH
		SET @ERROR_ID = ERROR_NUMBER();
